...
# 地方台频道（含23省、5自治区、4直辖市，对应34省级行政区地方台）
bj_lines = []          # 地方台-北京频道
tj_lines = []          # 地方台-天津频道
sh_lines = []          # 地方台-上海频道
cq_lines = []          # 地方台-重庆频道
...
# 其他及去重辅助
other_lines = []       # 未分类频道
other_lines_url = []   # 去重辅助容器
# favorite_lines = []   # 收藏频道（预留）
...
def process_channel_line(line):
    ...
    if "#genre#" not in line and "#EXTINF:" not in line and "," in line and "://" in line:
        ...
        if channel_address not in combined_blacklist:
            ...
            elif any(tyss_dictionary_item in channel_name for tyss_dictionary_item in tyss_dictionary) and check_url_existence(tyss_lines, channel_address):
            ...
            elif channel_name in xz_dictionary and check_url_existence(xz_lines, channel_address):
                xz_lines.append(process_name_string(line.strip()))
...
if __name__ == "__main__":
    ...
    # 加载频道字典文件（主频道+地方台+特色频道）
    # 主频道
    ys_dictionary = read_txt_to_array('scripts/livesource3/主频道/CCTV.txt')          # 中央频道排序
    ws_dictionary = read_txt_to_array('scripts/livesource3/主频道/卫视频道.txt')      # 卫视频道过滤+排序
    ty_dictionary = read_txt_to_array('scripts/livesource3/主频道/体育频道.txt')      # 体育频道过滤+排序
    tyss_dictionary = read_txt_to_array('scripts/livesource3/主频道/体育赛事.txt')    # 体育赛事过滤+排序
    sz_dictionary = read_txt_to_array('scripts/livesource3/主频道/数字频道.txt')      # 数字频道过滤+排序
    yy_dictionary = read_txt_to_array('scripts/livesource3/主频道/音乐频道.txt')      # 音乐频道过滤+排序
    gj_dictionary = read_txt_to_array('scripts/livesource3/主频道/国际频道.txt')      # 国际频道过滤+排序
    js_dictionary = read_txt_to_array('scripts/livesource3/主频道/解说频道.txt')      # 解说频道过滤+排序
    cw_dictionary = read_txt_to_array('scripts/livesource3/主频道/春晚频道.txt')      # 春晚频道过滤+排序
    dy_dictionary = read_txt_to_array('scripts/livesource3/主频道/电影频道.txt')      # 电影频道过滤+排序
    dsj_dictionary = read_txt_to_array('scripts/livesource3/主频道/电视剧频道.txt')    # 电视剧频道过滤+排序
    gat_dictionary = read_txt_to_array('scripts/livesource3/主频道/港澳台频道.txt')    # 港澳台频道过滤+排序
    xg_dictionary = read_txt_to_array('scripts/livesource3/主频道/香港频道.txt')      # 香港频道过滤+排序
    aomen_dictionary = read_txt_to_array('scripts/livesource3/主频道/澳门频道.txt')    # 澳门频道过滤+排序
    tw_dictionary = read_txt_to_array('scripts/livesource3/主频道/台湾频道.txt')      # 台湾频道过滤+排序
    dhp_dictionary = read_txt_to_array('scripts/livesource3/主频道/动画片频道.txt')    # 动画片频道过滤+排序
    douyu_dictionary = read_txt_to_array('scripts/livesource3/主频道/斗鱼直播.txt')    # 斗鱼直播过滤+排序
    huya_dictionary = read_txt_to_array('scripts/livesource3/主频道/虎牙直播.txt')    # 虎牙直播过滤+排序
    radio_dictionary = read_txt_to_array('scripts/livesource3/主频道/收音机频道.txt')  # 收音机频道过滤+排序
    zb_dictionary = read_txt_to_array('scripts/livesource3/主频道/直播中国.txt')      # 直播中国过滤+排序
    zy_dictionary = read_txt_to_array('scripts/livesource3/主频道/综艺频道.txt')      # 综艺频道过滤+排序
    game_dictionary = read_txt_to_array('scripts/livesource3/主频道/游戏频道.txt')      # 游戏频道过滤+排序
    xq_dictionary = read_txt_to_array('scripts/livesource3/主频道/戏曲频道.txt')      # 戏曲频道过滤+排序
    jlp_dictionary = read_txt_to_array('scripts/livesource3/主频道/记录片频道.txt')    # 记录片频道过滤+排序

    # 地方台频道（34省级行政区）
    bj_dictionary = read_txt_to_array('scripts/livesource3/地方台/北京频道.txt')        # 北京频道过滤+排序
    tj_dictionary = read_txt_to_array('scripts/livesource3/地方台/天津频道.txt')        # 天津频道过滤+排序
    sh_dictionary = read_txt_to_array('scripts/livesource3/地方台/上海频道.txt')        # 上海频道过滤+排序
    cq_dictionary = read_txt_to_array('scripts/livesource3/地方台/重庆频道.txt')        # 重庆频道过滤+排序
    heb_dictionary = read_txt_to_array('scripts/livesource3/地方台/河北频道.txt')        # 河北频道过滤+排序
    shanxi_dictionary = read_txt_to_array('scripts/livesource3/地方台/山西频道.txt')      # 山西频道过滤+排序
    ln_dictionary = read_txt_to_array('scripts/livesource3/地方台/辽宁频道.txt')        # 辽宁频道过滤+排序
    jl_dictionary = read_txt_to_array('scripts/livesource3/地方台/吉林频道.txt')        # 吉林频道过滤+排序
    hlj_dictionary = read_txt_to_array('scripts/livesource3/地方台/黑龙江频道.txt')      # 黑龙江频道过滤+排序
    jsu_dictionary = read_txt_to_array('scripts/livesource3/地方台/江苏频道.txt')        # 江苏频道过滤+排序
    zj_dictionary = read_txt_to_array('scripts/livesource3/地方台/浙江频道.txt')        # 浙江频道过滤+排序
    ah_dictionary = read_txt_to_array('scripts/livesource3/地方台/安徽频道.txt')        # 安徽频道过滤+排序
    fj_dictionary = read_txt_to_array('scripts/livesource3/地方台/福建频道.txt')        # 福建频道过滤+排序
    jx_dictionary = read_txt_to_array('scripts/livesource3/地方台/江西频道.txt')        # 江西频道过滤+排序
    shandong_dictionary = read_txt_to_array('scripts/livesource3/地方台/山东频道.txt')    # 山东频道过滤+排序
    hen_dictionary = read_txt_to_array('scripts/livesource3/地方台/河南频道.txt')        # 河南频道过滤+排序
    hb_dictionary = read_txt_to_array('scripts/livesource3/地方台/湖北频道.txt')        # 湖北频道过滤+排序
    hn_dictionary = read_txt_to_array('scripts/livesource3/地方台/湖南频道.txt')        # 湖南频道过滤+排序
    gd_dictionary = read_txt_to_array('scripts/livesource3/地方台/广东频道.txt')        # 广东频道过滤+排序
    hain_dictionary = read_txt_to_array('scripts/livesource3/地方台/海南频道.txt')      # 海南频道过滤+排序
    sc_dictionary = read_txt_to_array('scripts/livesource3/地方台/四川频道.txt')        # 四川频道过滤+排序
    gz_dictionary = read_txt_to_array('scripts/livesource3/地方台/贵州频道.txt')        # 贵州频道过滤+排序
    yunnan_dictionary = read_txt_to_array('scripts/livesource3/地方台/云南频道.txt')    # 云南频道过滤+排序
    xz_dictionary = read_txt_to_array('scripts/livesource3/地方台/西藏频道.txt')        # 西藏频道过滤+排序
    sx_dictionary = read_txt_to_array('scripts/livesource3/地方台/陕西频道.txt')        # 陕西频道过滤+排序
    gs_dictionary = read_txt_to_array('scripts/livesource3/地方台/甘肃频道.txt')        # 甘肃频道过滤+排序
    qh_dictionary = read_txt_to_array('scripts/livesource3/地方台/青海频道.txt')        # 青海频道过滤+排序
    nx_dictionary = read_txt_to_array('scripts/livesource3/地方台/宁夏频道.txt')        # 宁夏频道过滤+排序
    xj_dictionary = read_txt_to_array('scripts/livesource3/地方台/新疆频道.txt')        # 新疆频道过滤+排序
    nm_dictionary = read_txt_to_array('scripts/livesource3/地方台/内蒙古频道.txt')      # 内蒙古频道过滤+排序
    gx_dictionary = read_txt_to_array('scripts/livesource3/地方台/广西频道.txt')        # 广西频道过滤+排序


    # 加载频道名称纠错字典（与上方代码缩进层级一致）
